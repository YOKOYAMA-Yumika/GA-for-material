...
"""
POPULATIONS = inp_ga.POPULATIONS #population par generation
NUM_OF_STRINGS = inp_ga.NUM_OF_STRINGS #num of genes
GENERATIONS = inp_ga.MAX_GENERATIONS #number of generations
...
"""
num = int(POPULATIONS*SURVIVAL_RATE) #Number of individuals to be kept alive
...
def set_calc_place():
    calc_place = []
    for i in range(1, inp_ga.POPULATIONS+1):
...
def check_calc_status():
    while True:
        ...
        if len(finish_list) == POPULATIONS:
            return ["finish"]
        elif wait_list == []:
            return ["nowait"]
...
"""
def create_first_generation(POPULATIONS, genoms):
    """Read GENOMS and create POPULATIONS genes (initial population)"""
    generations = []
    first_gens = []
    index_indiv = 0
    ...
    if thread == True:
        futures = []
        with ThreadPoolExecutor(max_workers=ncore) as tpe:
...
def mutate(selected, children):
    ...
    num_of_mutation = POPULATIONS - int(POPULATIONS * CR_2PT_RATE / 2)*2 - int(POPULATIONS * CR_UNI_RATE / 2)*2
    count_mutation = 0
    j = 0
    while True:
...
def crossover(selected):
    """perform crossover"""
    children = []
    num_of_cross_2pt = int(POPULATIONS * CR_2PT_RATE / 2)
    num_of_cross_uni = int(POPULATIONS * CR_UNI_RATE / 2)
    count_2ptcross = 0
    count_unicross = 0
    k = 0
...
if __name__ == "__main__":
    ...
    if mode == "-ga":
        ...
        if RESTART == False:
            ...
            generation = create_first_generation(POPULATIONS, genoms)      
        else:       
